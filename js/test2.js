...
$(function () {
    $multPrint.click(function () {
        ...
        // 提取数据
        const dataList = chooseData.map(x => Object.keys(x)
        ...
        for (var i = 0; i < 1; i++) {
            ...
            for (var name in chooseData[0]) {
                if (name == 'barcode') {
                    ...
                    // 根据数据 count值重复生成
                    for (var j = 1; j < chooseData[0].count; j++) {
                        newBarCode = pageBarCode.clone()
                        console.log(`该标签count为${chooseData[0].count},正在循环生成第${j}个`)
                        ...
                        JsBarcode("#BarCode" + nums, chooseData[0][name], {
                            format: 'CODE128',
                            height: $("#EditBarCodeHeight").val() || $("#BarCodeHeight").val() || 30,
                            width: $("#EditBarCodeWidth").val() || $("#BarCodeWidth").val() || 2,
                            font: 'Sans-serif',
